...
async(conn, mek, m, { args, isOwner, reply, botNumber, config }) => {
    if (!isOwner) return reply("*YEH COMMAND SIRF AFTAB RKS KE LIYE HAI 😎*");
...
async(conn, mek, m, { args, isOwner, reply, botNumber, config }) => {
    if (!isOwner) return reply("*YEH COMMAND SIRF AFTAB RKS KE LIYE HAI ☺️*");
    const value = args[0]?.toLowerCase();
    ...
    if (value === 'on' || value === 'true') {
        ...
        reply(`*ANTI-CALL STATUS :❯ ${config.ANTI_CALL === "true" ? "ON" : "OFF"} 📵*\n\n*INCOMING CALLS AUTOMATICALLY REJECT HO JAYENGI 😃 YE SETTING ON KARNE K LIE LIKHO ☺️*\n*👑 ❮ANTICALL ON❯ 👑*\n*ANTICALL OFF KARNE K LIE LIKHO ☺️*\n*👑 ❮ANTICALL OFF❯ 👑*`);
    }
});
