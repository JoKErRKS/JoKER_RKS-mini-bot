...
// ==================== INDIVIDUAL JOIN REQUEST COMMANDS ====================
async function handleJoinRequestAction(conn, from, action, target, reply) {
    const requests = await conn.groupRequestParticipantsList(from);
    if (!requests.length) {
        return reply(`ℹ️ No pending join requests to ${action}.`);
    }

    let selected = requests;
    if (target) {
        selected = requests.filter(r => r.jid === target || r.jid.split('@')[0] === target.split('@')[0]);
        if (!selected.length) return reply("❌ That user does not have a pending join request.");
    }

    await conn.groupRequestParticipantsUpdate(
        from,
        selected.map(r => r.jid),
        action === "accept" ? "approve" : "reject"
    );

    return reply(`✅ ${action === "accept" ? "Accepted" : "Rejected"} ${selected.length} join request${selected.length === 1 ? "" : "s"}.`);
}

cmd({
    pattern: "accept",
    alias: ["approve"],
    desc: "Accept pending group join request(s)",
    category: "group",
    react: "✅",
    filename: __filename
}, async (conn, mek, m, { from, isGroup, isAdmins, isBotAdmins, mentionedJid, quoted, reply }) => {
    try {
        if (!isGroup) return reply("❌ This command only works in groups.");
        if (!isAdmins) return reply("❌ Only group admins can use this command.");
        if (!isBotAdmins) return reply("❌ I need to be admin to manage join requests.");
        const target = mentionedJid?.[0] || quoted?.sender || null;
        return await handleJoinRequestAction(conn, from, "accept", target, reply);
    } catch (e) {
        console.error("Accept request error:", e);
        return reply("❌ Failed to accept join request(s).");
    }
});
...
cmd({
    pattern: "reject",
    alias: ["deny"],
    desc: "Reject pending group join request(s)",
    category: "group",
    react: "❌",
    filename: __filename
}, async (conn, mek, m, { from, isGroup, isAdmins, isBotAdmins, mentionedJid, quoted, reply }) => {
    try {
        ...
        if (!isBotAdmins) return reply("❌ I need to be admin to manage join requests.");
        const target = mentionedJid?.[0] || quoted?.sender || null;
        return await handleJoinRequestAction(conn, from, "reject", target, reply);
    } catch (e) {
        console.error("Reject request error:", e);
        return reply("❌ Failed to reject join request(s).");
    }
});
