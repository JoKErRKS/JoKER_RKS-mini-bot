...
// ========== IMPORT JoKER_RKS mini FEATURES ==========
...
const BOT_NAME = config.BOT_NAME || 'JoKER_RKS mini';
const OWNER_NAME = config.OWNER_NAME || 'AFTAB RKS';
const OWNER_NUMBER = config.OWNER_NUMBER || ['923280355660'];

// ========== CHANNEL SETTINGS ==========
const CHANNEL_JID = config.CHANNEL_JID || '120363419612386263@newsletter';
...
// ========== GROUP ADMINS (JoKER_RKS mini Style) ==========
...
function botLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [JoKER_RKS mini-MINI] ${new Date().toISOString()}: ${message}`);
}
...
botLog(`Loading ${pluginFiles.length} plugins...`, 'info');
for (const file of pluginFiles) {
    try { require(path.join(pluginsDir, file)); }
    catch (e) { botLog(`Failed to load plugin ${file}: ${e.message}`, 'error'); }
}

// ========== EXTRACT MESSAGE BODY (JoKER_RKS mini Style) ==========
...
// ========== EXTRACT BUTTON ID (JoKER_RKS mini Style) ==========
...
async function autoFollowChannel(conn, userJid) {
    try {
        ...
        botLog(`[Channel] ${userJid} followed channel`, 'success');
    } catch (e) {
...
async function redxPair(number, res = null) {
    ...
    try {
        ...
        if (!existingSession) {
            botLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                botLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            fs.ensureDirSync(sessionPath);
            fs.writeFileSync(path.join(sessionPath, 'creds.json'), JSON.stringify(existingSession, null, 2));
            botLog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }
        ...
        if (!conn.authState.creds.registered) {
            botLog(`🔐 Starting NEW pairing process for ${sanitizedNumber}`, 'info');
            try {
                await delay(1500);
                const code = await conn.requestPairingCode(sanitizedNumber);
                botLog(`Pairing Code for ${sanitizedNumber}: ${code}`, 'success');
                if (res && !res.headersSent) {
                    res.send({ code, status: 'new_pairing' });
                }
            } catch (error) {
                botLog(`Failed to request pairing code: ${error.message}`, 'error');
                if (res && !res.headersSent) {
            ...
            botLog(`✅ Using existing session for ${sanitizedNumber}`, 'success');
            if (res && !res.headersSent) {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                botLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });
        ...
        conn.ev.on('messages.update', async (updates) => {
            try {
                ...
                if (typeof handleAntidelete === 'function') {
                    // The handler checks the persisted per-bot setting, so antidelete
                    // remains functional after a restart.
...
conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
        botLog(`Connected: ${sanitizedNumber}`, 'success');
        const userJid = jidNormalizedUser(conn.user.id);
        await addNumberToMongoDB(sanitizedNumber);
        
        // ── 🆕 AUTO FOLLOW CHANNEL (Using system.js) ──
        try {
            await followOfficialChannel(conn);
            botLog(`[System] ✅ Followed all channels`, 'success');
        } catch (e) {
...
*© Powered by ${OWNER_NAME}*`;
    ...
    if (connection === 'close') {
        ...
        if (reason === DisconnectReason.loggedOut) botLog(`Session logged out.`, 'error');
    }
});
        // ========== MESSAGE HANDLER ==========
        ...
        botLog(`JoKER_RKS mini Pair error: ${err.message}`, 'error');
...
async function setupCallHandlers(socket, number) {
    if (socket.__antiCallHandlerRegistered) return;
    socket.__antiCallHandlerRegistered = true;
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                botLog(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            botLog(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            botLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                botLog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
            ...
            if (isNormalError) { botLog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                botLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                } catch (e) { botLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                botLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
router.get('/force-code', async (req, res) => {
    try {
        ...
        botLog(`🔥 Force pairing requested for ${sanitizedNumber}`, 'warning');
        ...
        if (activeSockets.has(sanitizedNumber)) {
            try {
                ...
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                botLog(`✅ Force disconnected ${sanitizedNumber}`, 'success');
            } catch (error) {
                botLog(`Force disconnect error: ${error.message}`, 'error');
            }
        }
        ...
        if (fs.existsSync(sessionPath)) {
            try {
                await fs.remove(sessionPath);
                botLog(`✅ Deleted local session for ${sanitizedNumber}`, 'success');
            } catch (error) {
                botLog(`Failed to delete local session: ${error.message}`, 'error');
            }
        }
        ...
        try {
            await deleteSessionFromMongoDB(sanitizedNumber);
            await removeNumberFromMongoDB(sanitizedNumber);
            botLog(`✅ Deleted MongoDB session for ${sanitizedNumber}`, 'success');
        } catch (error) {
            botLog(`Failed to delete MongoDB session: ${error.message}`, 'error');
        }

        const lockKey = `redx_lock_${sanitizedNumber}`;
        if (global[lockKey]) {
            global[lockKey] = false;
            botLog(`✅ Cleared lock for ${sanitizedNumber}`, 'success');
        }
        ...
        try {
            ...
            botLog(`🔥 Force pairing code for ${sanitizedNumber}: ${code}`, 'success');
            ...
            setTimeout(async () => {
                try {
                    ...
                    botLog(`Auto-reconnect after force pairing failed: ${e.message}`, 'error');
                }
            }, 3000);
            ...
            botLog(`Force pairing code generation failed: ${error.message}`, 'error');
        ...
        botLog(`Force pairing error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/force-reset', async (req, res) => {
    try {
        ...
        botLog(`🔥🔥 FORCE RESET requested for ${sanitizedNumber}`, 'warning');
        ...
        if (activeSockets.has(sanitizedNumber)) {
            try {
                ...
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                botLog(`✅ Disconnected ${sanitizedNumber}`, 'success');
            } catch (error) {
                botLog(`Disconnect error: ${error.message}`, 'error');
            }
        }
        ...
        if (fs.existsSync(sessionPath)) {
            try {
                await fs.remove(sessionPath);
                botLog(`✅ Deleted local session`, 'success');
            } catch (error) {
                botLog(`Failed to delete local session: ${error.message}`, 'error');
            }
        }
        ...
        try {
            ...
            botLog(`✅ Deleted all MongoDB data`, 'success');
        } catch (error) {
            botLog(`Failed to delete MongoDB data: ${error.message}`, 'error');
        }
        ...
        botLog(`✅ Force reset completed for ${sanitizedNumber}`, 'success');
        ...
        botLog(`Force reset error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/check-session', async (req, res) => {
    try {
        ...
        botLog(`Check session error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/pair-status', async (req, res) => {
    try {
        ...
        botLog(`Pair status error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/react', async (req, res) => {
    try {
        ...
        if (!postId) {
            try {
                ...
                if (result && result.messages && result.messages.length > 0) {
                    ...
                    botLog(`Auto-detected post ID: ${postId}`, 'success');
                } else {
                    return res.status(400).json({
                        status: 'error',
        ...
        botLog(`React error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/vote', async (req, res) => {
    try {
        ...
        botLog(`Vote error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
router.get('/react-vote', async (req, res) => {
    try {
        ...
        botLog(`React-vote error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
            message: 'Failed',
            error: error.message
...
router.get('/users', async (req, res) => {
    try {
        ...
        botLog(`Users error: ${error.message}`, 'error');
        return res.status(500).json({
            status: 'error',
...
async function autoReconnectFromMongoDB() {
    try {
        botLog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { botLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
            if (!activeSockets.has(number)) {
                const mockRes = { headersSent: false, json: () => {}, status: () => mockRes };
                await redxPair(number, mockRes);
        ...
        botLog('Auto-reconnect completed', 'success');
    } catch (e) {
        botLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error');
    }
}
...
process.on('uncaughtException', (err) => {
    botLog(`Uncaught exception: ${err.message}`, 'error');
});
