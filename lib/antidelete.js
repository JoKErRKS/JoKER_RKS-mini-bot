...
async function handleAntidelete(conn, updates, store, botNumber) {
    try {
        // Prefer per-bot persisted settings, with global config as fallback.
        let userConfig = {};
        try { userConfig = await getUserConfigFromMongoDB(botNumber); } catch (_) {}
        if ((userConfig.ANTIDELETE ?? config.ANTIDELETE) !== 'true') return;

        // Get bot owner numbers from config
        const ownerNumbers = Array.isArray(config.OWNER_NUMBER) ? config.OWNER_NUMBER : [config.OWNER_NUMBER].filter(Boolean);
        ...
        for (const update of updates) {
            ...
            if (update.update && update.update.message) {
                ...
                if (message.protocolMessage && message.protocolMessage.type === 0) {
                    ...
                    const deleteId = `${deletedMessageKey?.remoteJid || ''}:${deletedMessageKey?.id || ''}`;
                    if (!deletedMessageKey?.remoteJid || !deletedMessageKey?.id || seen.has(deleteId)) continue;
                    seen.add(deleteId);
